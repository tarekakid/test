        ...
        <div class="row">
            <div class="col-md-6">
                <form runat="server" method="POST" action="{{ url('tag-material/' . $material->id) }}"
                    ...
                    <div class="input-group mb-3">
                        <select class="form-select @error('tag_id') is-invalid @enderror" id="selectAddTag"
                            aria-label="Добавьте автора" name="tag_id">
            ...
            <div class="col-md-6">
                <div class="d-flex justify-content-between mb-3">
                    <h3>Ссылки</h3>
                    <a type="button" class="btn btn-primary" data-bs-toggle="modal" id="addlink"
                        ...
                        @foreach ($links as $link)
                            <li class="list-group-item list-group-item-action d-flex justify-content-between">
                                <a href="{{ $link->link }}" class="me-3">
                                    {{ $link->signature == null ? $link->link : $link->signature }}
                                </a>
                                <input type="text" class="form-control d-none" placeholder="Напишите подпись ссылки"
                                    id="url" id="floatingSignature" value="{{ $link->id }}">
                                <span class="text-nowrap">
                                    <a href="#" type="{{ $link->id }}"
                                        class="open-AddUrlDialog text-decoration-none me-2 " data-bs-toggle="modal"
                                        data-bs-target="#exampleModal" data-id="{{ $link->link }}"
                                        id="{{ $link->signature }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                    ...
                                    <a href={{ url('delete-link/' . $link->id) }}" class="text-decoration-none"
                                        onclick="return confirm('Are you sure?')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                                            <path
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form runat="server" method="POST" action="{{ url('add-link/' . $material->id) }}"
                        ...
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control bookId @error('link')
                                is-invalid
                            @enderror" placeholder="Напишите ссылку"
                                id="floatinglink" name="link">
